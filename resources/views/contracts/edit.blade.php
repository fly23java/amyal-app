...
@section('content')
    ...
    <div class="card text-bg-theme">
         ...
         <div class="card-header d-flex justify-content-between align-items-center p-3">
            <h4 class="m-0">{{ trans('contracts.edit') }}</h4>
            <div>
...
<script>
    $(document).ready(function () {
        ...
        fetchDataAndPopulateDropdowns({{$contract->id}}, detailCount)
        ...
        function validateContractDetails() {
            ...
            $('.contract-detail').each(function () {
                ...
                if ($.trim(vehicleType.val()) === '' || $.trim(goods.val()) === '' || $.trim(price.val()) === '' || $.trim(dispersalCity.val()) === '' || $.trim(loadingCity.val()) === '') {
                    // يتم فحص الحقول للتأكد من أنها ليست فارغة فقط بالفعل
            ...
            return isValid;
                    ...
                    // في fetchVehicleTypes و fetchCity و fetchGoods
            function fetchDataAndPopulateDropdowns(contractId, targetRow) {
                $.ajax({
                    url: "{{ url('contracts/contracts/details/') }}/"+ contractId,
                    
                    method: 'GET',
                    success: function (response) {
                        const contractDetails = response.contractDetails;

                        $.each(contractDetails, function (index, currentDetail) {
                            updateFieldsWithData(currentDetail, targetRow);
                            targetRow = createNewDetail(); // إنشاء صف جديد للبيانات التالية
                ...
                // Fetch and populate vehicle types for the new detail
                        ...
                        // بعد ذلك، يمكنك استخدام الدوال الحالية (fetchVehicleTypes و fetchCity و fetchGoods) لاسترجاع باقي القيم
                        fetchVehicleTypes(targetRow);
                    },
                    error: function (error) {
            ...
            function updateFieldsWithData(data, targetRow) {
                // تحديث القيم في targetRow باستخدام البيانات المسترجعة من قاعدة البيانات (data)
                targetRow.find('.vehicle-type').val(data.vehicle_type);
                targetRow.find('.goods').val(data.goods_type);
                // ... تحديث الحقول الأخرى بناءً على احتياجاتك
            }

            function createNewDetail() {
                // الكود الحالي لإنشاء صف جديد
                const newDetail = $('#contract-details-container .contract-detail:first').clone();
                ...
                // إزالة أي فئات Select2 السابقة وتهيئتها
                newDetail.find('.select2').removeClass('select2-hidden-accessible').next().remove();
                newDetail.find('.select2').select2();
