@extends('layouts.dashbord-layout')

@section('content')

<div class="card text-bg-theme">

     <div class="card-header d-flex justify-content-between align-items-center p-3">
        <h4 class="m-0">{{ isset($title) ? $title : 'Vehicle Type' }}</h4>
        <div>
            <form method="POST" action="{!! route('vehicle_types.vehicle_type.destroy', $vehicleType->id) !!}" accept-charset="UTF-8">
                <input name="_method" value="DELETE" type="hidden">
                {{ csrf_field() }}

                <a href="{{ route('vehicle_types.vehicle_type.edit', $vehicleType->id ) }}" class="btn btn-primary" title="{{ trans('vehicle_types.edit') }}">
                    <i data-feather='edit'></i>
                </a>

                <button type="submit" class="btn btn-danger" title="{{ trans('vehicle_types.delete') }}" onclick="return confirm(&quot;{{ trans('vehicle_types.confirm_delete') }}?&quot;)">
                    <span class="fa-regular fa-trash-can" aria-hidden="true"></span>
                </button>

                <a href="{{ route('vehicle_types.vehicle_type.index') }}" class="btn btn-primary" title="{{ trans('vehicle_types.show_all') }}">
                    <span class="fa-solid fa-table-list" aria-hidden="true"></span>
                </a>

                <a href="{{ route('vehicle_types.vehicle_type.create') }}" class="btn btn-secondary" title="{{ trans('vehicle_types.create') }}">
                    <span class="fa-solid fa-plus" aria-hidden="true"></span>
                </a>

            </form>
        </div>
    </div>

    <div class="card-body">
        <dl class="row">
            <dt class="text-lg-end col-lg-2 col-xl-3">{{ trans('vehicle_types.name_arabic') }}</dt>
            <dd class="col-lg-10 col-xl-9">{{ $vehicleType->name_arabic }}</dd>
            <dt class="text-lg-end col-lg-2 col-xl-3">{{ trans('vehicle_types.name_english') }}</dt>
            <dd class="col-lg-10 col-xl-9">{{ $vehicleType->name_english }}</dd>

        </dl>

    </div>
</div>

@endsection